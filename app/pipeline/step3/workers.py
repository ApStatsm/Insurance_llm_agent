...
from app.services.multi_policy_service import (
    build_multi_policy_analysis,
    build_multi_policy_answer,
    expanded_policy_query,
    infer_product_category,
    merge_multi_policy_into_diagnosis,
    metadata_text,
    policy_doc_debug_items,
    product_keywords,
...
def _query_focus_keywords(query: str) -> list[str]:
    return query_focus_keywords(query)


def _expanded_policy_query(query: str, product_name: str) -> str:
    return expanded_policy_query(query, product_name)
...
def _policy_doc_debug_items(docs: list[Any]) -> list[dict[str, Any]]:
    return policy_doc_debug_items(docs)
...
def _run_policy_diagnosis_worker(
    ...
    multi_policy_analysis = build_multi_policy_analysis(
        customer=customer,
        question=query,
        route="policy_diagnosis",
    ...
    diagnosis_result = merge_multi_policy_into_diagnosis(diagnosis_result, multi_policy_analysis)
    if multi_policy_analysis.get("enabled"):
        ...
        draft = {
            ...
            "content": _append_document_analysis_summary(build_multi_policy_answer(multi_policy_analysis), doc_analysis_summary),
            "citations": citations,
