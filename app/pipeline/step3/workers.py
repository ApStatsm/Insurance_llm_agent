...
def _multi_policy_reason(policies: list[dict[str, Any]], question: str) -> str:
    names = [p.get("product_name", "가입상품") for p in policies]
    joined = ", ".join(names)
    if any(token in question for token in ("교통사고", "차 사고", "차량 사고", "차도", "자동차")) and any(
        token in question for token in ("병원", "치료", "진료비", "치료비", "실손", "실비")
    ):
...
def _build_policy_rag_query(question: str, policy: dict[str, Any]) -> str:
    product_name = _to_text(policy.get("product_name"))
    category = infer_product_category(product_name)
...
def _build_policy_coverage_assessment(
    *,
    ...
    checklist_missing = (claim_checklist or {}).get("missing_docs") or []
    missing_info = list(dict.fromkeys([*missing_by_category.get(category, []), *_to_text_list(checklist_missing)[:3]]))
    return {
        "status": status,
        "label": label,
...
def _to_text_list(items: Any) -> list[str]:
    ...
    for item in values:
        if isinstance(item, dict):
            labels.append(_to_text(item.get("label") or item.get("doc") or " 또는 ".join(item.get("any_of") or [])))
        else:
            labels.append(_to_text(item))
    ...
    *,
    ...
    if vectorstore is not None:
        try:
            ...
            debug["documents"] = _policy_doc_debug_items(retrieved_docs)
...
def _policy_next_actions(category: str, checklist: dict[str, Any]) -> list[str]:
    missing = _to_text_list((checklist or {}).get("missing_docs"))
    actions: list[str] = []
    if missing:
...
def _build_multi_policy_analysis(
    *,
...
def _merge_multi_policy_into_diagnosis(
...
def _build_multi_policy_answer(multi_policy_analysis: dict[str, Any]) -> str:
    policy_results = multi_policy_analysis.get("policy_results") or []
    ...
    for idx, result in enumerate(policy_results, start=1):
        ...
        first_evidence = evidence_cards[0] if evidence_cards else {}
        evidence_label = (
        ...
        required_docs = _to_text_list(checklist.get("required_docs") or checklist.get("missing_docs"))
        missing_info = _to_text_list(assessment.get("missing_info"))
        lines.extend(
            [
...
def _run_policy_diagnosis_worker(
    ...
    multi_policy_analysis = _build_multi_policy_analysis(
        customer=customer,
        question=query,
        route="policy_diagnosis",
        vectorstore=vectorstore,
        rule_db=rule_db,
        uploaded_docs=updated.get("user_docs") or [],
    )
    diagnosis_result = _merge_multi_policy_into_diagnosis(diagnosis_result, multi_policy_analysis)
    if multi_policy_analysis.get("enabled"):
        citations = [
            {
                "source_id": f"MULTI{idx}",
                "reference": (result.get("evidence_cards") or [{}])[0].get("article_number")
        ...
        draft = {
            ...
            "content": _append_document_analysis_summary(_build_multi_policy_answer(multi_policy_analysis), doc_analysis_summary),
            "citations": citations,
            "diagnosis_result": diagnosis_result,
            "debug": {**debug_info, "multi_policy_analysis": multi_policy_analysis},
        ...
        updated["audit_log"].append(
            build_audit_event(
                ...
                action="MULTI_POLICY_DRAFT_CREATED",
                note=f"policies={len(multi_policy_analysis.get('policy_results') or [])}",
            )
        )
        return updated
...
def _run_document_claim_worker(
    ...
    final_content = _build_multi_policy_answer(multi_policy_analysis) if multi_policy_analysis.get("enabled") else "\n".join(summary_lines)

    draft = {
        "worker_type": "document_claim",
        "content": _append_document_analysis_summary(final_content, doc_analysis_summary),
        ...
        "debug": {
            ...
            "multi_policy_analysis": multi_policy_analysis,
        },
        "created_at": updated["updated_at"],
    }
    patch = {
